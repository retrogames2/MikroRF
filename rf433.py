...
menu = ["came_bruteforce", "rf433jammer"]
selected = 0
old_selected = -1

def drawmenu(menu, selected):
    tft.fill(st7789.BLACK)
    tft.text(font32, "433MHZ RF TOOLS:", 40, 10, st7789.GREEN)
    tft.text(font816, "kliknij lewo aby wyjsc <-", 10, 220, st7789.WHITE)
    for index, item in enumerate(menu):
        y = 60 + index * 36
        if index == selected:
            tft.text(font32, item, 30, y, st7789.WHITE)
            tft.rect(20, y - 4, 290, 40, st7789.BLUE)
        else:
            tft.text(font32, item, 30, y, st7789.WHITE)
